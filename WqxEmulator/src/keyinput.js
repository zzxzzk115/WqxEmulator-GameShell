var WqxKeyInput = function (){
    ...
    WqxKeyInput.prototype._mkeyDownOrUp = function (key, downOrUp){
        var keyName = keyCodeToKeyName[key];
        var wqxKeyCode = keyNameToKeypadMatrixIndex[keyName];
        if (wqxKeyCode) {
            var col = wqxKeyCode >> 3;
            var row = wqxKeyCode & 0x07;
            this.wqx.keypadmatrix[row][col] = downOrUp ? 1 : 0;
    ...
    WqxKeyInput.prototype.mkeyDown = function (key){
        this._mkeyDownOrUp(key, true);
        this.onpress(key);
    };
    WqxKeyInput.prototype.mkeyUp = function (key){
        this._mkeyDownOrUp(key, false);
        this.onrelease(key);
    };
